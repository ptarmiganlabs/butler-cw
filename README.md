...
The downside is that when a user opens a large Sense app, it takes significant time to load the app from the file server into Qlik's associative engine (a.k.a. QIX) of the server that the user is connected to.
...
* a config file (YAML) used by the Node.js app to know where to find server certificates etc.
* a separate config file (YAML) for specifying with what frequency what apps should be loaded into which servers.
* a Docker image that wrap the previous three parts into a single image, which can be executed in any Docker environment. Functionally there is no difference in using native Node.js or Docker - just that the latter is usually a lot more convenient. 
...
This file contains sensitive information about where to find the Sense server certificates used when connecting to the Sense servers, and other parameters used to run the actual cache warming service. This file is stored on local disk right next to the Node.js code for the cache warmer itself.
...
1. Make sure you have a recent version of [Node.js](https://nodejs.org) installed. Node 8.11.4 was used during development of the most recent Butler CW version.
...
The `production.yaml` file can also be named anything, as long as it matches the value of the NODE_ENV environment variable matches.  
...
Start the service by running "node index.js". Butler CW has been tested on both macOS,  Windows Server 2012 R2, Windows Server 2016, Debian and Ubuntu.

### Running in a Docker container

This is in almost all ways preferred over running Butler CW as a native Node.js app:
...
appConfig:
  ...
  github:
    ...
    password: <secret personal GitHub token>
    owner: ptarmiganlabs
    repo: butler-cw
    path: config/butler-cw_apps.yaml

# Is connection to Sense engine secure (https)?
isSecure: true

# Directory where Butler CW's log files should stored (no trailing / )
logDirectory: ./log
proton:butler-cw-docker goran$
...
services:
  butler-cw:
    ...
    environment:
      - "NODE_ENV=production"
    logging:
      driver: json-fileproton:butler-cw-docker goran$
proton:butler-cw-docker goran$
```
